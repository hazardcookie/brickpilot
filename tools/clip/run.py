...
def migrate_messages(messages):
  """Migrate route log messages when the local native deps allow it.

  `process_replay.migration` imports planner modules that depend on generated
  acados extensions. Developer macOS worktrees often have Linux-built copies of
  those extensions in-tree, which makes the clip renderer fail before it can do
  any local UI replay. For current local review routes the raw log schema is
  already renderable, so fall back to the original messages if that optional
  migration stack cannot be imported.
  """
  global _migration_warning_logged
  try:
    from openpilot.selfdrive.test.process_replay.migration import migrate_all
    return migrate_all(messages)
  except ImportError as e:
    if not _migration_warning_logged:
      logger.warning("Log migration unavailable; rendering clip with raw messages: %s", e)
...
def ffmpeg_executable() -> str:
  for env_key in ("FFMPEG_BINARY", "FFMPEG"):
    if os.getenv(env_key):
      return os.environ[env_key]
  # `uv run` puts the repo virtualenv first on PATH.  Its minimal ffmpeg build
  # can fail to decode qcamera MPEG-TS from stdin on macOS, so prefer the full
  # Homebrew binary when it is available.
...
def setup_env(output_path: str, big: bool = False, speed: int = 1, target_mb: float = 0, duration: int = 0, headless: bool = True):
  os.environ.update({"RECORD": "1", "RECORD_OUTPUT": str(Path(output_path).with_suffix(".mp4")), "FFMPEG_BINARY": ffmpeg_executable()})
  if headless:
...
def _parse_and_chunk_segment(args: tuple) -> list[dict]:
  ...
  messages = migrate_messages(list(_LogFileReader("", dat=raw_data, sort_by_time=True)))
  if not messages:
    return []
  ...
  for global_idx in range(start_frame, end_frame):
    ...
    if seg_idx != current_seg:
      ...
      if use_qcam:
        ...
        with FileReader(path) as f:
          result = subprocess.run([ffmpeg_executable(), "-v", "quiet", "-i", "-", "-f", "rawvideo", "-pix_fmt", "nv12", "-"],
