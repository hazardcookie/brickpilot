...
TUCSON_PHEV_RESEARCH = CarTuningConfig(
  v_ego_starting=0.15,
  # 0.2.2-beta: May 9 drive 3/4 labels showed the 0.2.1.x stop-creep
  # tune entered/held stopping too aggressively while catch-up launches were
  # still sluggish. Keep a Tucson-specific anti-creep bias over stock hybrid,
  # but back off the sticky stopping threshold/decel ramp and let positive
  # accel converge faster.
  v_ego_stopping=0.75,
  stopping_decel_rate=0.65,
  lookahead_jerk_upper_v=[0.35, 0.45, 0.58],
  lookahead_jerk_lower_v=[0.28, 0.36, 0.60],
)

CAR_SPECIFIC_CONFIGS = {
  # Local Tucson PHEV stop-creep research tune for Dan's NX4 PHEV.
  CAR.HYUNDAI_TUCSON_4TH_GEN: TUCSON_PHEV_RESEARCH,
