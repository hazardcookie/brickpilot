...
from opendbc.sunnypilot.car.hyundai.longitudinal.helpers import get_car_config, get_longitudinal_tune
...
class TestLongitudinalTuningController(unittest.TestCase):
  ...
  def test_tucson_phev_research_config(self):
    self.CP.carFingerprint = CAR.HYUNDAI_TUCSON_4TH_GEN
    config = get_car_config(self.CP)
    self.assertEqual(config.v_ego_starting, 0.15)
    self.assertEqual(config.v_ego_stopping, 0.75)
    self.assertEqual(config.stopping_decel_rate, 0.65)
    self.assertEqual(config.lookahead_jerk_upper_v, [0.35, 0.45, 0.58])
    self.assertEqual(config.lookahead_jerk_lower_v, [0.28, 0.36, 0.60])

    get_longitudinal_tune(self.CP)
    self.assertEqual(self.CP.vEgoStarting, 0.15)
    self.assertEqual(self.CP.vEgoStopping, 0.75)
    self.assertEqual(self.CP.stoppingDecelRate, 0.65)
