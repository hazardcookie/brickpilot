...
class AlertRenderer(Widget):
  def __init__(self):
    ...
    self._full_text2_label = Label("", font_size=ALERT_FONT_BIG, text_alignment=rl.GuiTextAlignment.TEXT_ALIGN_CENTER,
                                   text_alignment_vertical=rl.GuiTextAlignmentVertical.TEXT_ALIGN_TOP)
    self._suppressed_event_names: set[str] = set()

  def set_suppressed_event_names(self, event_names: set[str] | None) -> None:
    self._suppressed_event_names = event_names or set()

  def _is_suppressed(self, alert: Alert) -> bool:
    event_name = alert.alert_type.split('/')[0] if alert.alert_type else ""
    return event_name in self._suppressed_event_names
  ...
  def get_alert(self, sm: messaging.SubMaster) -> Alert | None:
    ...
    alert = Alert(text1=ss.alertText1, text2=ss.alertText2, size=ss.alertSize.raw, status=ss.alertStatus.raw, alert_type=ss.alertType)
    return None if self._is_suppressed(alert) else alert
