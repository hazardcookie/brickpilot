...
from openpilot.selfdrive.ui.feedback.bookmark_tag_prompt import BookmarkTagPrompt
...
class AugmentedRoadView(CameraView):
  def __init__(self,
    ...
    self._bookmark_icon = BookmarkIcon(self._handle_bookmark_triggered)
    self._bookmark_tag_prompt = BookmarkTagPrompt(bookmark_tag_callback)
  ...
  def _handle_mouse_release(self, mouse_pos: MousePos):
    # Don't trigger click callback if bookmark/tag UI was triggered
    if not self._bookmark_icon.interacting() and not self._bookmark_tag_prompt.interacting():
      super()._handle_mouse_release(mouse_pos)

  def _handle_bookmark_triggered(self) -> None:
    bookmark_log_mono_time = self._bookmark_callback() if self._bookmark_callback else None
    self._bookmark_tag_prompt.show(bookmark_log_mono_time)
  ...
  def _render(self, _):
    ...
    suppressed_alerts = {"userBookmark"} if self._bookmark_tag_prompt.showing_buttons() else set()
    self._alert_renderer.set_suppressed_event_names(suppressed_alerts)
    ...
    self._bookmark_tag_prompt.render(self._content_rect)
