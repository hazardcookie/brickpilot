...
from openpilot.system.version import CUSTOM_BRAND_NAME, CUSTOM_BRAND_VERSION, RELEASE_BRANCHES
...
class MiciHomeLayout(Widget):
  def __init__(self):
    ...
    self._openpilot_label = UnifiedLabel(CUSTOM_BRAND_NAME, font_size=96, font_weight=FontWeight.DISPLAY, max_width=480, wrap_text=False)
  ...
  def _get_version_text(self) -> tuple[str, str, str, str]:
    branch = ui_state.params.get("GitBranch") or ""
    commit = ui_state.params.get("GitCommit") or ""
    ...
    return CUSTOM_BRAND_VERSION, branch, commit[:7], date_str
