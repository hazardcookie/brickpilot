...
ffmpeg are available, then emits metadata + a small browser UI that appends Dan's
labels to a local JSONL file through a localhost-only Python server.
"""
...
def _clip_anchor_route_time(event: dict[str, Any]) -> tuple[float, str]:
    ...
    Human bookmarks are reactions after Dan noticed something, so for labelled
...
def create_openpilot_ui_clip_if_possible(event: dict[str, Any], out_dir: Path, timeout_sec: float) -> tuple[str, str]:
    ...
    try:
        proc = subprocess.run(cmd, cwd=ROOT, env=env, capture_output=True, text=True, timeout=max(30.0, timeout_sec))
...
def write_ui(out_dir: Path) -> None:
    ...
    readme = README_MD.replace("analysis/drive_tests/review_queue_20260512_batch2", relpath(out_dir).rstrip("/"))
...
def write_report(out_dir: Path, queue: list[dict[str, Any]], inspection: dict[str, Any], args: argparse.Namespace) -> None:
    ...
    lines = [
        ...
        "# LAN option, if Dan wants to open from phone on the same WiFi:",
        ...
        f"Clip timing now defaults to {args.seconds_before:.0f}s lookback and {args.seconds_after:.0f}s after the selected marker. For bookmark/tag-driven items, the selected marker is the human bookmark because Dan's labels are after-the-fact reactions; clip metadata exposes route start/end, event offset, bookmark offsets, and source segments.",
        "",
        "## Queue composition",
        "",
    ]
    ...
    lines += [
        ...
        "## Selection strategy",
        "",
        "The generator samples high-priority, diverse events rather than asking Dan to review every event:",
        "",
        ...
        "- After Dan reviews labels, ingest `reviews.jsonl` into the Brickpilot 0.5.0 labeling/training analysis loop.",
    ]
...
Default binding is loopback-only. Use --host 0.0.0.0 only when Dan wants LAN
...
```bash
cd analysis/drive_tests/review_queue_20260512_batch2
python3 review_server.py --port 8765
```
...
```bash
python3 scripts/drive_tests/review_queue.py --out analysis/drive_tests/review_queue_20260512_batch2 --limit 60
```
