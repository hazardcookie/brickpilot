...
def generate_review_video(route_uuid: str, *, config: str | None = None, output_root: Path | None = None) -> dict:
    ...
    existing = store.one(
        """SELECT a.id, a.artifact_path
           ...
           WHERE ra.route_uuid=? AND a.kind='full_drive_video'
    ...
    output_root = output_root or Path(os.environ.get("BRICKPILOT_LABELER_OUTPUT_ROOT", "/Users/brick/BrickpilotDriveDB/labeler_outputs"))
